...
module.exports = {
    ...
    fillSelectPicker: function (filterName, selectPicker, objectArray) {
        ...
        if (["activityGroup", "activity", "treatmentMethodGroup", "treatmentMethod"].includes(filterName)) {
            objectArray.forEach(item => itemsHtml += "<option value='" + item.attributes.id + "'>" + enrichFlows.returnCodePlusName(item) + "</option>");
        } else if (filterName == "month") {
            objectArray.forEach(month => itemsHtml += "<option value='" + month.attributes.id + "'>" + month.attributes.code.substring(2, 6) + " " + utils.toMonthString(month.attributes.code.substring(0, 2)) + "</option>");
        } else if (filterName == "waste04") {
